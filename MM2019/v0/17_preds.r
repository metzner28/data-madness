...
dfT = read.csv('NCAATourneyDetailedResults.csv') %>% filter(Season == 2017)
...
df = rbind(dfTeamA, dfTeamB) %>%
  ...
  mutate(Conf = sapply(TeamID, get_conf, Season = 2017))
...
write.csv(matchups, 'results2017.csv', row.names = FALSE)
