...
for (n in 1:100) {
  ...
  preds_xgb = predict(model_xgb, newdata = df_test_x)
  loss_xgb = logLoss(df_test$y, preds_xgb)
  xgb[n] = loss_xgb
  
  # fit random forest model, hyperparameter tuning by hand
  model_rf = ranger(y ~ ., data = df_train, num.trees = 500, max.depth = 2)
  preds_rf = predict(model_rf, data = df_test_x)$predictions
  loss_rf = logLoss(df_test$y, preds_rf)
  rf[n] = loss_rf
  
  # fit lasso glm
  model_glmnet = cv.glmnet(x = df_train_x, y = df_train$y)
  preds_glm = predict(model_glmnet, newx = df_test_x, s = "lambda.1se")
  loss_glm = logLoss(df_test$y, preds_glm)
  glm[n] = loss_glm
